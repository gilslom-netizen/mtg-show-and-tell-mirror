...
describe('the PIMC agent', () => {
  ...
  /**
   * §9.4, pooled over a handful of games rather than asserted on one.
   *
   * The measured median branching factor is two and most of those are "pass, or tap
   * a land"; if search fired on all of them it would cost fifty times what it does
   * and buy nothing. But the ratio in a *single* game swings from about 0.07 to 1.0,
   * because a game that ends on turn three gives the agent five decisions and nearly
   * all of them are real ones. Asserting on one seed measured the seed, not the
   * agent: swapping one card in the main deck moved the chosen seed from 0.33 to
   * 0.39 without anything about the search changing. Pooling is the claim that was
   * always meant.
   */
  it(
    ...
    () => {
      ...
      let decisions = 0;
      let playouts = 0;
      for (const seed of [52003, 52004, 52005, 52006, 52007]) {
        const { stats } = smallGame(seed);
        searched += stats.searched;
        decisions += stats.decisions;
        playouts += stats.playouts;
      }
      expect(searched).toBeLessThan(decisions * 0.35);
