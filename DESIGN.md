...
> המסמך הזה נכתב כתכנון. הטבלה הבאה מתעדת מה נבנה בפועל, ומה נותר.
> קראו גם את [`README.md`](README.md) להוראות הרצה.

| שלב | מצב | הערות |
|---|---|---|
| 0 · תשתית | ✅ | חבילת npm אחת במקום monorepo — ראו הערת הסטייה בסעיף 4.2 |
| 1 · ליבת המנוע | ✅ | תור, פריוריטי, סטאק, SBA, קרב |
| 2 · קרקעות | ✅ | fetch, shock, surveil, MDFC, תנאי כניסה |
| 3 · מנוע מאנה | ✅ | פותר עם backtracking + גיזום; פתרון של 10 מאנה ב־<1ms |
| 4 · קלפים פשוטים | ✅ | |
| 5 · Show and Tell + Omniscience | ✅ | `simultaneousSecret` + כניסה בו־זמנית |
| 6 · אינטראקטיביים | ✅ | Mana Drain, Bowmasters, Veil, Mistrise, Sanctuary |
| 7 · שרת | ✅ | חדרים, redaction, reconnect עם token, לוג פעולות מתמיד |
| 8 · לקוח MVP | ✅ | |
| 9 · שכבת הנוחות | ✅ | auto-pass, TriggerPolicy, מצב Omniscience, ראש ספרייה ידוע, קיצורים |
| 10 · Bo3 | ⚠️ חלקי | מבנה המאצ', בחירת play/draw וסטטיסטיקות — **בלי סיידבורד** (ראו למטה) |
| 11 · Replay ו־Lab Mode | ❌ | הנתונים קיימים (seed + action log), אין עדיין ממשק נגן |

**מה לא נבנה בכוונה:**

- **סיידבורדינג.** ששת קלפי הסייד נמצאים מחוץ למאגר הקלפים שמומש (המשתמש ביקש להתעלם
  מהסייד), ולכן מסך החלפה היה מסך שלא יכול לעשות כלום. כדי להוסיף אותו צריך לממש
  `Carpet of Flowers`, `Chrome Mox`, `Krosan Grip`, `Mystical Dispute` ו־`Thoughtseize`
  (`Veil of Summer` כבר קיים). ה־considering board נשאר כמודל נתונים בלבד.
- **Undo רחב.** `Esc` מבטל הטלה באמצע. Undo מלא צריך את שלושת תנאי ה־safe window שבסעיף 11.3.
- **נגן Replay ו־Lab Mode.** דורש רק ממשק — המנוע כבר דטרמיניסטי לחלוטין.

**באגים אמיתיים שהבדיקות תפסו במהלך הבנייה** (מתועדים גם בטבלת הסיכונים):
ספל שיצא מכל הזונות בזמן שהמתין לבחירה, התפוצצות אקספוננציאלית בפותר המאנה,
וקריסה כשטוקן תוקף מת לפני הכרזת חוסמים.

**באג שדווח מהשטח — שני טאבים שלא נפגשו.** שני שחקנים פתחו את המשחק, השאירו את
תיבת קוד החדר ריקה, וכל טאב ייצר לעצמו קוד אקראי אחר — כלומר שני חדרים נפרדים. שלוש
תקלות נפרדות הצטרפו לאותה מסך המתנה אילם:

1. הקוד נוצר רק כשהתיבה ריקה. עכשיו הוא נוצר לפני הלחיצה, מופיע ב־URL, ויש קישור
   הזמנה להעתקה — אי אפשר יותר "לפספס" זה את זה.
2. מסך ההמתנה לא הציג כלום — לא קוד, לא סטטוס חיבור, ובעיקר לא שגיאות (הטוסט של
   השגיאות ישב אחרי ה־early return). "החדר כבר מלא", סוקט מת ומארח בלי אחסון משותף
   נראו כולם בדיוק כמו חבר שעוד לא לחץ. עכשיו כל מה שידוע מוצג.
3. `npm run dev` לא הגיש `/api` בכלל, כך שהלקוח נפל חזרה ל־WebSocket שאף אחד לא
   הריץ. שרת הפיתוח מריץ עכשיו את אותם handlers של הפריסה, בתוך אותו תהליך.

בנוסף, `/api/health` מדווח `serverless` ו־`usable`, והלובי מסרב לפתוח משחק אונליין
על מארח serverless בלי Redis — שם שני השחקנים היו נוחתים על מופעים שונים ומחכים זה
לזה לנצח.

**ואז התברר שהפונקציות עצמן קרסו בוורסל** (`FUNCTION_INVOCATION_FAILED`), כך שהלקוח
נפל ל־WebSocket גם בפרודקשן. הסיבה: וורסל לא מאגדת (bundle) — היא מקמפלת כל קובץ
בנפרד ומריצה Node ESM נקי, ושם `import ... from './deck'` בלי סיומת לא נפתר, ו־
`import x from '...json'` דורש `with { type: 'json' }` שהטרנספיילר מוריד בדרך. שני
הדברים עבדו בכל כלי פיתוח (Vite, vitest, tsx) ונשברו רק בפרודקשן.

התיקון: כל ייבוא יחסי ב־`src/engine`, `src/server` ו־`api` נושא סיומת `.js` מפורשת,
ונתוני הקלפים מיובאים מ־TypeScript מגונרט ב־`src/engine/generated/` במקום מ־JSON
(`data/*.json` נשאר מקור האמת; טסט נכשל אם השניים מתפצלים). `src/client` נשאר כפי
שהיה — הוא נארז ב־Vite ולא רץ אף פעם ב־Node.

הלקח נשמר כסקריפט ולא כהערה: `npm run check:serverless` מקמפל קובץ־קובץ בלי אגד
וקורא לשני ה־handlers תחת Node רגיל — בדיוק הסביבה שהפילה את הפריסה — והוא חלק
מ־`npm run build`, כך שוורסל עצמה תסרב לבנות API שבור.
...
├─────────────┬────────────────────────────────────┬───────────┤
│             │   שדה היריב (יצורים/קסמים/קרקעות)  │  הסטאק    │
│  לוג        │                                    │  ▔▔▔▔     │
│  משחק       │                                    │  [ספל 2]  │
│  (מקופל)    ├════════════ קו אמצע ═══════════════┤  [ספל 1]  │
│             │   השדה שלי  [מאנה צפה: ⓒⓒⓒ]        │           │
...
הקרקעות היו פעם שורה נפרדת בגודל שני שליש, ושתי השורות האלה עלו יותר ממה
שחסכו: שתי שורות הן שתי הזדמנויות לגלוש לשורה שנייה, וחצי־לוח מוקטן לפי מה
ששתיהן צריכות יחד. חמש קרקעות ושני קבועים אחרים תפסו שתי שורות ו־160 פיקסלים
כדי להראות שבעה קלפים שנכנסים לרוחב של רבע מהמסך. היום כל הקבועים של שחקן הם
שורה אחת, באותו גודל, והקרקעות ממוינות לכיוון קו האמצע — כך ששני הלוחות עדיין
פונים זה אל זה, אבל הלוח נכנס בגודל קלף גדול יותר.

חלוקת הגובה בין שני החצאים נגזרת ממה שיש על כל אחד מהם (`useFitBoard`), ולא
מ־`align-content: stretch` של ה־CSS שחילק שווה בשווה. החלוקה השווה הייתה התשובה
הלא נכונה מהתור הראשון — רק לחצי העליון יש רצועת יד של היריב מעליו — והיא זו
שייצרה את שתי הצורות של אותה תלונה בבת אחת: הצד העמוס נחתך, והצד השקט החזיק
רצועה ריקה מתחת לפס הנתונים. חצי שנשאר לו מקום גדל לתוכו (עד פי 1.3) במקום
להשאיר אותו ריק.
